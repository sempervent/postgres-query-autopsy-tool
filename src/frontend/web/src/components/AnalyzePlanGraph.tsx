...
function GraphToolbar({ selectedNodeId }: { selectedNodeId: string | null }) {
  ...
  return (
    ...
    >
      <button onClick={fit} style={{ padding: '6px 10px', borderRadius: 10, cursor: 'pointer' }}>
        Fit
      </button>
      <button
        onClick={focusSelected}
        disabled={!selectedNodeId}
        style={{ padding: '6px 10px', borderRadius: 10, cursor: selectedNodeId ? 'pointer' : 'not-allowed', opacity: selectedNodeId ? 1 : 0.6 }}
      >
        Focus
      </button>
      <button onClick={reset} style={{ padding: '6px 10px', borderRadius: 10, cursor: 'pointer' }}>
        Reset
      </button>
      <details>
        <summary style={{ cursor: 'pointer', opacity: 0.85 }}>Legend</summary>
