...
{
    public static AnalysisNarrative From(PlanSummary summary, IReadOnlyList<AnalysisFinding> rankedFindings)
    {
        ...
        var whereTimeWent = summary.TopExclusiveTimeHotspotNodeIds.Count > 0
            ? $"Top exclusive-time hotspots: {string.Join(", ", summary.TopExclusiveTimeHotspotNodeIds.Take(5))}."
        ...
        var whatMatters = summary.HasBuffers && summary.TopSharedReadHotspotNodeIds.Count > 0
            ? $"Top shared-read hotspots: {string.Join(", ", summary.TopSharedReadHotspotNodeIds.Take(5))}."
