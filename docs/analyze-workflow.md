...
**Phase 72 (copy reliability):** **Copy reference** / **Copy share link** / **Copy suggested EXPLAIN** use **`copyToClipboard`** (**Clipboard API** when available, **`document.execCommand('copy')`** fallback for non-secure origins) and **`useCopyFeedback`** shows success or a short **copy-failed** hint. **`ClickableRow`** ignores clicks that originate on nested **`button`** / **`a`** / **`[data-pqat-row-no-activate]`** so copy does not flip navigator selection. **`nodeReferenceText`** appends a compact **`· node …`** suffix with the canonical planner **node id** for tickets and deep links. Playwright smoke (**Phase 77**) stubs **`navigator.clipboard.writeText`** in the page and asserts **Analyze → Copy reference** sends the expected reference text (does not rely on **`readText()`** in CI). Backend **`PostgresJsonAnalyzeFixtureSweepTests`** exercises the full **`PlanAnalysisService`** pipeline over every top-level **`fixtures/postgres-json/*.json`** (see [Fixtures](fixtures.md)).
...
2. **Summary + metadata** — compact summary line, **Plan briefing** (Phase 60–64: structured fields as **story lanes** in the summary deck—orientation, work concentration, drivers, **Inspect first** tactical steps (often echoing **`operatorBriefingLine`** on the lead bottleneck when augmented), propagation/flow, index/shape—with **Focus** buttons still tied to canonical `nodeId`), **Plan source / EXPLAIN metadata**, and share/artifact copy when applicable.
3. **Plan workspace** — the graph (or text tree) is the visual center. **Responsive tiers** (`useWorkspaceLayoutTier`): **narrow** (&lt;900px) stacks the guide **below** the graph; **medium** (900–1319px) keeps **graph + Plan guide** side-by-side with slightly different column weighting than **wide** (≥1320px), which gives the graph and investigation surface more horizontal room. The **left** column holds plan mode toggles, search boxes, fit/focus/reset, and the tree or graph; the **right** column (when visible) is the **Plan guide** rail with:
   - **Focused operator** readout when a node is selected: human-readable title, **Phase 63–64** backend **`operatorBriefingLine`** (dense **Briefing** strip: primary label · join role when known · measurable pressure), join/branch subtitle when applicable, metrics, longer **What this operator is doing** interpretation, findings cue, and a collapsed **Technical id** for the canonical planner path
   ...
   - **Where to inspect next** — **Inspect first** lane when `inspectFirstPath` exists, then the same **hotspot** rows (click/keyboard selects the node; **Copy** stays separate)
