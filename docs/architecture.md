...
**Phase 82 (user-facing explanation + actionability):** **`PlanStoryBuilder`** uses clearer driver phrasing (**primary pressure types**), avoids repeating the top suggestion’s body in **`InspectFirstPath`** (points users at ranked **Optimization suggestions**), drops duplicate **operatorBriefingLine** embed in step 1 (briefing stays on bottleneck cards), and appends a bounded **`time_bucket`** hint when SQL suggests bucketing. **`ComparisonStoryBuilder`** extends the overview when faster root + worsened pairs or slower root + improved pairs coexist; tightens grouped-output continuity tails. Frontend **`storyPresentation`** renames plan/compare section eyebrows for scan-first reading; **Analyze** summary adds a short **reading order** hint; **Plan guide** hotspots no longer duplicate the full **Start here** paragraph; **suggestion** cards sort by **priority → confidence**, elevate **Try next**, dedupe identical summary/action lines, and mark the top leverage card; **selected node** shows **bottleneck posture** when the node is on a ranked bottleneck; **Compare** change briefing gains an interpretive subtitle. Markdown/HTML report plan-briefing labels align with the UI.

**Phase 83 (frontend CI reliability + structured next steps):** **`src/frontend/web/package.json`** **`optionalDependencies`** pins **`@rolldown/binding-*`** per platform (same version as the Rolldown stack) so **GitHub Actions** **`npm ci`** materializes **linux-x64-gnu** bindings for Vitest/Vite 8. **`PlanStory`** adds **`InspectFirstSteps`** (JSON **`inspectFirstSteps`**); **`InspectFirstPath`** remains a joined legacy string. **`PairRewriteVerdictBuilder`** + **`NodePairDetail.RewriteVerdictOneLiner`** give Compare a one-line, evidence-bound **rewrite outcome**; low mapping confidence prefixes **Weak mapping—** when metric clauses are still shown. **`PlanAnalysisService`** Markdown/HTML reports emit numbered **Start here** lists when steps exist. UI: **Start here** lane renders ordered steps + optional **Focus in plan**; suggestion cards add a **Bottleneck #N** jump chip when **`relatedBottleneckInsightIds`** resolves.

**Phase 84 (clipboard trust + reporting + issues):** **`copyToClipboard`** uses **synchronous `execCommand('copy')` first**, then **`navigator.clipboard.writeText`**, fixing user-gesture loss after **`await`** in strict browsers. **`useCopyFeedback`** uses **`globalThis.setTimeout`** and clearer failure copy; copy status regions expose **`aria-live`**. **`suggestionReferenceText`** + **Copy for ticket** on suggestion cards. **`POST /api/compare/report/html`** + **`RenderCompareHtmlReport`** (top pairs + **Rewrite outcome** when verdict exists). Root **`ISSUE.md`** + **`.github/ISSUE_TEMPLATE/bug_report.md`**. Vitest **`setup.ts`** / integration tests align with the real copy order; Playwright clipboard capture records both paths.

**Phase 86 (export parity + copy payloads + a11y):** Analyze **HTML** suggestions align with markdown (**id**, **Why**, **Validate**); **Headline Findings** typo fixed. Compare **HTML** adds **Next steps after this change** when compare suggestions exist; compare **markdown** suggestions gain **family** + optional **Why**. **`pairReferenceText`** / **`nodeReferenceText`** / **`suggestionReferenceText`** carry **`PQAT compare:`** / **`PQAT analysis:`** scope lines and clearer **Plan A/B node** + **Pair artifact** tails; **`compareDeepLinkClipboardPayload`** / **`analyzeDeepLinkClipboardPayload`** stack URL + ids for **Copy link**. Compare summary **Copy for ticket** on compare suggestions. Copy status regions add **`aria-atomic="true"`**; success dwell **~2.2s**. Tests: **`AnalyzeReportExportTests`**, extended **`CompareReportHtmlTests`**, Vitest **`shareAppUrl`**, **`nodeReferences`**, **`optimizationSuggestionsPresentation`**; Playwright adds **Compare deep link** + **suggested EXPLAIN** flows.

**Phase 87 (workflow polish + JSON parity + graph tests):** Vitest **`setup.ts`** fakes **`offsetWidth`/`offsetHeight`** inside **`.react-flow`**, implements **`ResizeObserver`** callbacks, uses **Lines** grid in tests only (**`AnalyzePlanGraphCore`**), guards **`setCenter`**, and briefly filtered a transient React **NaN-attribute** console line. **Phase 88** removed that global filter and hardened **`useCopyFeedback`** teardown so copy success timers do not throw after unmount. Compare markdown **Next steps** heading matches HTML. **`ReportJsonExportParityTests`** + heading parity test protect JSON/report shape. Playwright adds **Compare Copy for ticket** + **Analyze Copy share link**.

**Phase 89 (compare deep links + graph noise):** **`analyzeGraphAdapter`** + **`AnalyzePlanGraphCore`** coerce **finite** node positions; **`GraphSync`** defers **`fitView`** until the viewport is finite. **`AnalyzePage.interaction.test.tsx`** applies a **file-local** **`console.error`** filter for the known React **NaN SVG attribute** warning only. Playwright adds **Compare Copy link** reopen (**`pair=`** + **Rewrite outcome**) and **same-pair** suggestion **Copy for ticket** (**`Pair scope:`** line). Compare **Next steps** **`section`** landmark; **`useCopyFeedback`** unmount regression tests.

**Phase 90 (compare collaboration + semantics):** Compare **heading** ladder (**`h1`** capture → **`h2`** summary / navigator / pair / top changes); **Next steps** suggestion lines use **`h3`** titles with **Pin** after **Focus plan B** / **Copy for ticket** in **DOM** (keyboard-friendly **Tab** order). Playwright **`persisted-flows`**: **`suggestion=`** highlight **reopen** in a **second tab**; **`jwt-auth-smoke`**: **Compare Copy link** clipboard under **JWT**. **`AnalyzePlanGraphCore`** **`defaultViewport`**: explicit **`{ x:0, y:0, zoom:1 }`**; **NaN** stderr may still require the **Analyze interaction** file filter in Vitest.

**Phase 91 (frontend CI + proxy clipboard parity):** GitHub Actions **`frontend`** uses **`scripts/verify-frontend-docker.sh`** (same **`npm ci`**, **`fixtures:check`**, **Vitest**, **Vite build** as **`Makefile`** **`verify-frontend-docker`**). **`e2e-auth-proxy`**: **Compare Copy link** clipboard test aligns with **JWT**. Compare **Pin** **`aria-describedby`** suggestion title id; share vs pair **Copy** tooltips.

**Phase 92 (visual + finding deep link):** **`e2e-visual`** Analyze happy-path snapshot uses **`analyze-visual-summary-contract`** (metrics + structured plan briefing), excluding share/metadata blocks. Playwright **`persisted-flows`** covers **`finding=`** reopen highlight parity with **`suggestion=`**.

**Phase 93 (deep links + copy + jsdom graph):** **`indexDiff=`** reopen/highlight E2E ( **`compare-index-changes-callout`** ); **`finding=`** / **`indexDiff=`** URL assertions; **`suggestion=`** reopen asserts **settled** query preservation. **`shareAppUrl.compareDeepLinkClipboardPayload`** adds optional **Pinned …** lines. Vitest **`setup.ts`** patches **`getBoundingClientRect`** inside **`.react-flow`** when the rect is empty (complements offset stubs; **`AnalyzePage.interaction`** no longer filters **`console.error`** for NaN attributes).

**Phase 94 (pinned copy + export + suggestion docs):** Playwright **`Copy link with pinned index insight`** (clipboard + URL round-trip); **`indexDiff=`** **`toBeInViewport`** on **Index changes**; JWT smoke asserts **Pinned index insight** in auth mode. Compare **HTML** export gains **Index changes** (markdown heading unified to **`## Index changes`**). **`resolveCompareSuggestionParamToCanonicalId`** JSDoc + E2E seed comment clarify **`suggestion=`** / **`alsoKnownAs`** behavior.
...
- Analyze page: paste/upload plan JSON; **Phase 49** maps **422/409** artifact GET failures to explicit UI copy (corrupt vs unsupported schema vs not found vs access denied). **Phase 39+42** lay out results as a **workspace**: graph/text tree + **Plan guide** rail (`AnalyzePage`, **three-tier** layout via **`useWorkspaceLayoutTier`**: narrow &lt;900px stacks, **medium** 900–1319px keeps side-by-side graph+guide with tuned column ratios, **wide** ≥1320px emphasizes the investigation surface), then lower band for findings + suggestions + selected node; **Phase 97** makes **medium/wide** graph + guide a **single shared-height row** (**`pqat-analyzeWorkspaceRow--paired`**, **`companionRailSurfaceStyle`**, **`graphFillColumn`** on **`AnalyzePlanGraphCore`**, internal guide scroll) so the rail is not **`max-height`**-stranded beside a taller workspace. **Phase 98:** beside-workspace **Plan guide** may lift **Selection snapshot** into **`pqat-planGuideRail__stickyBand`**; **Compare** adds **`aria-live`** pin announcements (**`comparePinLiveAnnouncement`**) and Playwright keyboard coverage for **Index changes** + **Next steps** **Home**/**End**. **Phase 99:** **Text** mode in paired Analyze uses **`pqat-planTextTreeBand`**; **Plan guide** scroll body **`aria-label`** when beside workspace; **Compare** hydrate vs transition pin copy (**`comparePinHydrateAnnouncementForFingerprint`**), **`PIN_LIVE_ANNOUNCE_DEFER_MS`** vs **`COPY_FEEDBACK_SUCCESS_CLEAR_MS`**, shared keyboard **`aria-describedby`** hints, **`preventScroll`** on roving focus, and **multi-row** Index E2E (**`rewrite_sort_seq_shipments`** / **`rewrite_index_ordered_shipments`**). **Phase 100:** **`parseCompareUrlPinAndPairState`** resyncs pins + **`pair=`** when the query string changes without a new snapshot; hydrate **`aria-live`** lines auto-clear via **`useEffect`** + **`COMPARE_PIN_HYDRATE_CLEAR_MS`**; Playwright covers hydrate + **Space** pin + **indexDiff** URL churn. The graph is **`AnalyzePlanGraphLazy`** wrapping **`AnalyzePlanGraphCore`** (React Flow) in a **lazy chunk** with **`PlanGraphSkeleton`** while **Text** mode avoids loading that chunk until **Graph** is chosen (**idle** + **Graph** control **hover/focus** prefetch). **Phase 42** widens the app shell (fluid **`max-width`** in `App.css`). **Phase 43** applies **`workstation.css`** + design tokens. **Phase 44** adds **`workstation-patterns.css`**, **`React.lazy`** for **`AnalyzePage`**, and defers **`AnalyzeWorkspaceCustomizerInner`** until the customizer **`<details>`** opens (with **`RouteFallback`** / **`CustomizerBodyFallback`** loading UI). **Phase 45** adds **lower-band** lazy chunks (**findings** / **suggestions** / **selected node**), **`HeavyPanelShell`** + **`LowerBandPanelSkeleton`**, **`VirtualizedListColumn`** for long findings and suggestion lists, and **`AnalyzeSelectedNodeHeavySections`** lazy-loaded under the selected-node header. **Phase 48** keeps **family group headers** inside the virtualized optimization list via **flattened rows** + **`getItemSize`**, and normalizes **older persisted** suggestion JSON for display.
...
- `POST /api/compare/report/html` returns a compact HTML compare report; **Top worsened/improved** lines include **Rewrite outcome** when present, plus **plan capture (per side)**, **change briefing** / **story beats** when applicable, **bottleneck posture**, **narrative**, and **Next steps after this change** when compare suggestions exist (**Phase 88**).

**Phase 101 (onboarding / help UX):** Frontend **`src/frontend/web/src/help/`** — **`helpSurface.css`** (**`pqat-help-*`** tokens and shells, not **`pqat-panel`**), **`WorkflowGuideShell`**, **`WorkflowGuideBar`**, **`AnalyzeWorkflowGuide`**, **`CompareWorkflowGuide`**. **`App.tsx`** imports the stylesheet globally. Analyze/Compare pages own toggle state (empty vs loaded snapshot, Compare respects workspace **`intro`** default). **`pqat-help-inline`** contextual callouts live next to high-friction controls (capture, graph/text mode, compare copy actions). **`CompareIntroPanel`** removed; docs describe **`pair=`** removal semantics and a compact URL lifecycle matrix.

**Phase 102 (help discoverability):** **`workflowGuidePrefs.ts`** — **`pqat_workflow_guide_v1`** dismissal flags, **`WorkflowGuideQueryParam`** (`guide` = **`1`** / **`true`**), URL strip on guide close (transition guard). **`workflowGuideHotkey.ts`** — **`?`** / **Shift+/** when not typing in form fields. Playwright **`persisted-flows`** covers **`?guide=1`**, URL strip, Compare toggle, and hotkey ignore-in-input + synthetic **`?`** on **`window`**. Vitest covers prefs, hotkey targets, and **`data-pqat-help-surface`** on the guide shell.

**Phase 103 (help lifecycle / support):** **`workflowGuideEntryUrl.ts`** + **`WorkflowGuideCopyGuidedRow`** (guide shell **`footer`**). **`WorkflowGuideShell`** — focusable guide title + **`pqat-help-shell`** class contract. **`AnalyzePage` / `ComparePage`** — merged **`keydown`**: **`?`** opens with pending focus-to-title; **`Esc`** closes with dismissal + toggle refocus (ignored in text fields); explicit open/close uses **`pending*FocusTitleRef`** / **`explicit*ClosePendingRef`** so programmatic closes (e.g. after analyze/compare) do not steal focus. **`WorkflowGuideBar`** — **`toggleRef`**, native **`title`** tooltip for **`?` / Esc**. Playwright: reload persistence for Analyze + Compare, **Copy guided link** clipboard, **Esc** + focus return. Vitest: **`workflowGuideEntryUrl.test.ts`**, Esc + focus + **`pqat-help-shell`** class.

**Phase 104 (help a11y / guided URL):** **`workflowGuideFocusLoopHelpers.ts`** — collect focusables + **Tab** wrap handler; wired from **`WorkflowGuideShell`** when **`keyboardContain`** (explicit opens only). Guide title **`tabIndex={0}`** for natural tab order with footer button. **`buildCopyGuidedLinkUrlFromLocation`** merges **`guide=1`** into current **`Location`**. **`AnalyzePage` / `ComparePage`** — **`queue*GuideLiveMsg`**, **`data-testid="*-workflow-guide-announcer"`**. Vitest: **`workflowGuideFocusLoopHelpers.test.ts`**, merged URL tests, shell **not** **`pqat-panel`**, announcer interaction test. Compare E2E patches **`intro: true`** on existing workspace JSON when present.

**Phase 105 (help landmarks / guided-link clarity / browser announcer):** **`WorkflowGuideShell`** — explicit **`role="region"`**; footer **`role="group"`** **`aria-label="Guided link sharing"`**. **`WorkflowGuideCopyGuidedRow`** — **merged** + **entry** guided copy (**`buildCopyGuidedLinkUrlFromLocation`** vs **`buildWorkflowGuideAbsoluteUrl`**). **`ComparePage`** — **`?guide=`** no longer seeds initial **`useState`** (matches **`AnalyzePage`** / **`readAnalyzeGuideInitialOpen`** pattern). Playwright **`persisted-flows`**: announcer text for open/close/**`?guide=`** after dismiss; clipboard asserts for entry vs merged. Vitest: region/footer contract, **`?guide=`** announcer via **`MemoryRouter`**.

**Phase 106 (guided parity / first paint / visual contract):** **`workflowGuideOpenFromUrl.ts`** — **`openWorkflowGuideWhenUrlRequests`**; **Analyze** + **Compare** call it from **`useLayoutEffect`** (avoids **`?guide=`** one-frame flash; keeps announcer transition). **`WorkflowGuideShell`** — **`data-pqat-help-visual-contract`**. Playwright **`persisted-flows`**: **Compare** merged guided **`goto`** round-trip; **`e2e-visual`**: **`analyze-workflow-guide-shell.png`**. Vitest: **`workflowGuideOpenFromUrl.test.ts`**.

**Phase 107 (cognitive load / examples / triage):** **`src/examples/`** — curated **`?raw`** JSON for in-app samples; **`TryPlanExampleChips`** (**`-capture` / `-help`** test ids). **`analyzeOutputGuidance.ts`** / **`compareOutputGuidance.ts`** — **`analyzeTakeawayFromResult`**, **`compareLeadTakeaway`**. **Analyze** / **Compare** pages: **`onAnalyze(override?)`**, **`onCompare(overrideA?, overrideB?)`**. Summary UI: **`pqat-takeawayBand`**, collapsed **Plan source** + denser plan/compare story behind **`<details>`**; user-facing persistence wording.

**Phase 108 (scan-first / expanded samples):** More bundled **`src/examples/plans/*.json`** (three Analyze + two Compare). **`analyzeFollowUpScanSignals`**, **`compareFollowUpDiffSignals`**; **`analyze-scan-signals`** / **`compare-scan-signals`** UI; **`primaryTriageFindingId`** cascade to **Findings**; **`analyze-summary-heading`**, **`analyze-optimization-suggestions-panel`** for stable E2E; plan/compare story lanes further collapsed by default.

**Phase 109 (first-screen triage + continuity + compare scan):** **`buildAnalyzeTriageBundle`** unifies takeaway + scan signals; **`analyze-triage-deck`** + ranked **Then scan** strip + **`stickyTriageNarrow`** on narrow layouts; **selected node** + **Plan guide** **`triageEcho`** echo summary focus; **Findings** **`scrollIntoView`** for primary triage row (guarded for jsdom). **Compare** **`compare-triage-deck`**, **Show row** + **`scrollArtifactIntoView`** (**`block: 'center'`**), **`summaryStickyNarrow`**, more prose behind **`<details>`**. Fourth Analyze sample (**nested-loop / join-heavy**): **`nl-join-inner-heavy`** + fixture sync.

**Phase 110 (decision surface + continuity under load):** **`VirtualizedListColumn`** imperative **`scrollToIndex`** + **`motionPreferences`** (reduced-motion-aware scroll); **Analyze** triage row targeting works when findings are virtualized; **`compareTriagePairBridgeLine`** + **`compare-selected-pair-triage-bridge`**; **Analyze** suggestions **Start here** alignment chip; sticky **`--pqat-sticky-top-offset`**; calmer sharing/summary copy.

**Phase 111 (guided-path cohesion):** **`suggestionAlignsWithAnalyzeTriage`** widens **Analyze** suggestion ↔ **Start here** ties beyond **`relatedFindingIds`**; **Plan guide** jump to primary finding row; **selected node** top-next cue when aligned; **`useStickyTopOffsetSync`** derives sticky offset from **`.topBar`**. **Compare** bridge covers **change story** beats; **navigator** briefing cues (**`briefingHighlightPair`**); pair panel **Context** / **Suggested follow-up** chain; product-facing copy on summary, sharing, and scan actions (**Show in list**).

**Phase 112 (triage continuity + calmer surfaces):** **`firstVirtualRowIndexAlignedWithAnalyzeTriage`** + **`AnalyzeOptimizationSuggestionsPanel`** **`scrollToIndex`** keep **Start-here–aligned** suggestions visible when lists virtualize. **`triagePrimaryRoute`** gates **Plan guide** CTAs (**finding row** vs **jump to plan focus**). **`analyzeExportTriage`** augments **client-side** analyze exports (MD/HTML/JSON) with a compact triage envelope; **`shareAppUrl`** / **`artifactLinks`** adjust clipboard pinned lines. **Compare** **`continuityPairFallback`**, **Reading thread** summary phrasing, and **`compareOutputGuidance`** URL-bridge wording improve coherence without URL-only dead air. **`artifactErrorPresentation`** + capture loading copy reduce system tone. Sticky shells add **`safe-area-inset-top`** atop measured top-bar offset.

**Phase 113 (guided cohesion + Compare export parity):** **`compareExportTriage.ts`** + **`exportCompareMarkdown` / `exportCompareHtml` / `exportCompareJson`** (**`api/client.ts`**) prepend/inject **`pqatExportCompareTriage`** on Compare downloads when plan text is present. **`resolveComparePairFallbackDisplay`** + **`compareContinuityCueIsSpecific`** tune pair-panel fallback copy. **Plan guide** triage echo dedupes headline vs summary; **ArtifactErrorBanner** shows optional **`artifactErrorBannerNextStep`**. **`VirtualizedListColumn`** supports **`scrollContainerTestId`**; Playwright proves long virtualized suggestions path.

**Phase 114 (reopen/export cohesion + landing proof):** **`POST /api/compare/report/*`** accepts optional **`comparison`** body (full **`PlanComparisonResultV2`**) via **`CompareExecution.RunForReportAsync`**—report rendering skips re-compare when the snapshot is supplied. The SPA sends **`buildCompareReportRequestBody`** snapshot mode when plan inputs are empty after **`GET /api/comparisons/{id}`**. **`CompareCapturePanel`** uses per-format busy labels; **`resolveComparePairFallbackDisplay`** drops vague filler; **Analyze** guide echo differentiates **overview** scan strip vs finding/step threads; Playwright asserts **viewport + scroller bounds** for **`analyze-suggestion-card-triage-aligned`**.

**Phase 115 (guided export parity + trust):** **`PlanAnalysisService.RenderCompareMarkdownReport` / `RenderCompareHtmlReport`** emit a compact **Reading thread** lead (**`BuildCompareReadingThreadLead`**) so exports are not client-only. **`nginx.conf`** raises **`client_max_body_size`** for **`/api/`**; Playwright uses **`waitForRequest`** (not **`page.route`**) for large export POSTs. **`compareExportTriage.ts`** adds **`markdownCompareExportSupplement`** / **`injectCompareExportSupplementIntoHtml`** for selection-only context. **`Compare: reopen with empty plan inputs exports markdown using snapshot payload`**; **`AnalyzePlanGuideRail`** hides triage echo unless **Also scan** chips or a primary jump action exists; calmer **`artifactErrorBannerTitle`** strings.

**Phase 116 (Analyze export parity + trust surfaces):** **`ArtifactPersistenceJson.ApplyToHttpSerializerOptions`** centralizes HTTP JSON alignment with persistence options. **`AnalyzeCapturePanel`** adds a **Download** band (**`analyze-export-*`**) with snapshot cue, busy labels, and status text; **`formatApiErrorResponse`** improves export **`4xx`** copy; **`filterTriageEchoScanLabels`** dedupes guide **Also scan** chips vs **Start here**; **`compareContinuityCueIsSpecific`** rejects a few vague short tokens. Playwright **`Analyze: reopen with empty plan input exports markdown using snapshot payload`**.

**Phase 117 (export failure clarity + parity):** **`UseExceptionHandler`** in **`Program.cs`** returns JSON **400** (**`request_body_invalid`** / **`export_request_incomplete`**) or **500** (**`internal_error`**) for failures on **`/api/report/*`** and **`/api/compare/report/*`** instead of opaque empty bodies. **`formatApiErrorResponse`** surfaces server **`message`** lines for those export codes without echoing raw **`error`** tokens. Compare export success hints align with Analyze; export status regions add polite live announcements; **Plan guide** overview echo omits redundant framing when **Also scan** chips are present. Backend **`ReportExportInvalidBodyApiTests`** lock the contract.

**Phase 118 (stable contracts + ProblemDetails):** **`ReportExportValidation.TryAnalyzeReportRequest`** returns **`Results.BadRequest`** for **`export_request_incomplete`** (no **`BadHttpRequestException`** / message sniffing). **`ReportExportBadRequestExceptionHandler`** (**`IExceptionHandler`**) maps **`BadHttpRequestException`** on report paths to **`request_body_invalid`**. **`AddProblemDetails`** + **`UseExceptionHandler()`** replace the bespoke catch-all **500** JSON block. **`ReportExportValidationTests`** + extended Playwright (**Compare** reopen→**HTML** export, **Analyze** visible export-failure status) + **`formatApiErrorResponse`** **413** JSON codes. **`ArtifactSharingPanel`** copy pass.

**Phase 119 (export parity + take-with-you UX):** Playwright proves **Analyze** reopen→**HTML** and **Compare** reopen→**JSON** snapshot exports; capture panels use **Take with you** eyebrow and shorter hints; **`formatApiErrorResponse`** calms **ProblemDetails**-style **500** bodies; **Plan guide** triage echo suppresses redundant headline whenever **Also scan** chips exist; sharing save confirmation **Sharing updated.**

**Phase 120 (final reopen parity + handoff):** Playwright adds **Analyze** reopen→**JSON** (**`POST /api/report/json`**); **Take with you** bands add a one-line Markdown/HTML/JSON legend (**`analyze-export-format-legend`** / **`compare-export-format-legend`**); **ArtifactSharingPanel** **`artifact-sharing-effect-note`** + **Sharing saved…** copy; **`filterTriageEchoScanLabels`** / **`compareContinuityCueIsSpecific`** reduce weak duplicate guidance; empty-body **400** export failures get a clearer **`formatApiErrorResponse`** line.

**Phase 121 (action surfaces + trust copy):** **`pqat-handoffBand`** / **`exportDownloadSuccessHint`** unify Analyze/Compare export success; **`formatApiErrorResponse`** handles empty **401**/**413** consistently; **ArtifactSharingPanel** post-save + effect note polish; **`compareContinuityCueIsSpecific`** short **faster/plan** path + tests for **`resolveComparePairFallbackDisplay`**.

**Phase 122 (readability + graph→finding thread):** Smaller default graph frame; Analyze paired grid shifts weight toward the plan guide column; **`topRankedFindingForNode`** + **`graphPivotFindingId`** link graph selection to the findings list; **Selected node** **`analyze-related-findings-bridge`** chips; **`AnalyzeFindingsPanel`** graph pivot row styling; Vitest **`graph node click`** interaction test.

**Phase 123 (graph-local findings):** **`AnalyzeLocalFindingsShelf`** in **Plan workspace** + selected-node detail; **`rankedFindingsForNode`**; graph **`jumpToNodeId`** does not auto-scroll the ranked list — **`See in ranked list`** sets **`graphPivotFindingId`**; findings row cue **Opened from detail**.

**Phase 124 (dedupe + navigator):** **`AnalyzeLocalFindingsBridge`** when workspace + selected-node visible; shelf lead/secondary hierarchy + a11y; Playwright asserts graph click does not show ranked-list **Opened from detail** until explicit **See in ranked list**.

**Phase 125 (workspace reading path + local evidence):** **`pqat-investigationThread`** on selected-node readout; shelf **Why this operator matters** / **Strongest match** / **Full write-up in list**; bridge adapts single vs multi (`localEvidencePresentation` max severity); truncation cue under shelf when previews < total; findings intro aligned; Playwright proves **`analyze-local-evidence-open-top-in-list`** → **Opened from detail**.

**Phase 126 (flow + evidence consistency):** **`evidenceNavCopy`** + aria helpers; **`SkipToRankedFindingsLink`** → **`#analyze-ranked-findings`**; **`AnalyzeFindingsPanel`** + **`AnalyzePlanGuideRail`** use shared **`severityLabel`** / chips from **`localEvidencePresentation`**; shelf truncation + **Other titles**; responsive polish on investigation band.

**Phase 127 (focus + language parity):** Skip link **scroll + focus** on Ranked panel; **`graphPivotFindingId`** path **focus()** with retry on the ranked **`ClickableRow`** scoped under **`#analyze-ranked-findings`** (shelf preview **`li[data-finding-id]`** no longer shadows **`querySelector`**); **`findingConfidenceLabel`**; preview vs full **aria** cues; **`nav.pqat-planEvidenceShortcuts`**.

**Phase 128 (evidence semantics + arrival):** Preview shelf **`data-pqat-preview-finding-id`** vs ranked **`data-finding-id`**; **`queryRankedFindingRow`**; virtual **`scrollToIndex`** **`onSettled`**; pivot cue timer tied to **focus arrived** + **6s** fallback (Phase 129); graph-pivot row **focus-visible** + skip **:focus-within** stacking.

**Phase 129 (arrival confidence):** **`scheduleVirtualScrollSettled`** module + tests; ranked **arrival cue** + **`aria-describedby`** on **Findings** **`h2`**; graph legend repositioned vs skip link; pivot fallback **6s**; skip **`data-testid`** + E2E pointer path.

**Phase 130 (ranked continuation + signal):** **`pqat-rankedFindingsPanel--pivotContinuity`**; calmer left-stripe arrival cue; graph-pivot row wash; **`scroll-margin-top`** on **`#analyze-ranked-findings`**; Vitest **`getBBox`** shim + React SVG **NaN** console filter; **`AnalyzePlanGraphCore`** dots-only **Background**; skip E2E **focus + click**.

**Phase 131 (cross-surface continuation):** Analyze **Continues from plan** thread hint (trimmed duplicate operator / row cue); **`comparePairHandoffKind`** + **`compare-visual-pair-continuation-contract`**; **`e2e-visual`** **`analyze-ranked-continuation-contract`** + **`compare-pair-continuation-contract`**; NaN **`console.error`** filter gated on **`.react-flow`** in document.

**Phase 132 (responsive + restored continuity):** **`compareHandoffOrigin`** + **`comparePairHandoffDisplayText`** (**`comparePairHandoffCopy.ts`**); narrow **Compare** DOM order navigator→pair; narrow **Analyze** lower-band reading order; continuation **`role="region"`** + **`aria-labelledby`** wiring; Playwright reopen asserts **`data-pqat-handoff-origin`** **`link`**.

**Phase 133 (legible provenance + narrow flow):** **`Summary — reopened`** / **`Briefing — reopened`** for **`link`** + **`summary`**/**`briefing`**; **`SkipToPairInspectorLink`** → **`compare-pair-inspector-region`**; pair + Analyze ranked continuation **`aria-describedby`** on **`role="region"`** without duplicating thread text on **`h2`**; visual corrupt seed **retry**; Playwright visible reopen cue.

**Phase 134 (Analyze reopen parity + keyboard proof):** **`analyzeRankedHandoffOrigin`** + **`analyzeRankedContinuityCopy`** (**`Ranked — reopened`**, **`Continues from plan — reopened`**); Compare **`link`** **pinned**/**navigator** → **`Pinned — reopened`** / **`From the lists — reopened`**; Playwright narrow **`Enter`** on skip → **`#compare-pair-inspector-region`** focused; matrix Vitest + reopen E2E.

**Phase 135 (breadth + model):** **`withReopenedSuffix`** (**`reopenedContinuityCopy.ts`**); **`exportDownloadSuccessHint`** **`restoredFromLink`**; **`compareContinuityCueIsSpecific`** join-strategy shift gate for **Reading thread** pair fallback; Playwright **Briefing — reopened** (**NL→hash** fixtures + continuity panel) + narrow **Tab** chain to skip; compact handoff **matrix** in **`docs/compare-workflow.md`**.

**Phase 136 (stability + proof):** **`AnalyzePage`** analyze **`useLayoutEffect`** URL sync; **`compareContinuityCueSpecificity.ts`** + fixture table tests; Playwright **From the lists — reopened** (nested-loop compare fixtures) + **`toHaveURL`** gates before **`page.url()`** on Analyze/Compare persist paths.

**Phase 137 (graph-first issue readout):** **`AnalyzeGraphIssueSummary`** + **`graphNodeIssueSummaryPresentation`** under **`AnalyzePlanWorkspacePanel`** (**`pqat-graphInvestigationStack`**); workspace **`AnalyzeLocalFindingsShelf`** **`compactWorkspacePreview`** + demoted **`AnalyzeLocalFindingsBridge`**; default graph **`clamp(240px, 30vh, 420px)`**; Playwright graph-click asserts in-band summary and no Ranked focus until explicit pivot.
